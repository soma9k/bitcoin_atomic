...
class FaucetController < ApplicationController
    ...
    def index
        ...
        @utxo=utxos[0]
        @amount = utxos.sum { |u| u['amount'].to_f  }
        @histories = History.all
    end

    # Bitcoin送金メソッド
    def create
        tx = send_bitcoin(params[:to_address], params[:amount].to_f)
            if tx
                History.create(to_adress: params[:to_address], amount: params[:amount].to_f, txid: tx)
